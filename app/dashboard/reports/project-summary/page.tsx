...
export default function ProjectSummaryPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const other = await getDocs(collection(db, "projectOtherPayments"));
  ...
  /* ================= TOTALS ================= */
  const totalMemberIncome = filteredPayments.reduce(
    (s, p) => s + Number(p.amount || 0),
    0
  );

  const totalOtherIncome = filteredOtherIncome.reduce(
  ...
  const balance = totalMemberIncome + totalOtherIncome - totalExpense;
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 text-black">
      ...
      <div className="bg-white p-4 rounded-xl border shadow-sm mb-6 grid grid-cols-1 md:grid-cols-4 gap-3 print:hidden">
        ...
        <select value={unitId} onChange={e=>setUnitId(e.target.value)} className="border rounded px-3 py-2">
